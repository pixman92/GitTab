...
function makeNice(){
    ...
    //========================================
    for(var i=0; i<retrieved.single.tabs.length; i++){
        //required \" to make URL (a href) work!
        str.push(makeStr(['<div class="pure-u-1-3"><div class="fontSmall margMe"><div class="w3-card-2 innerPad"><div><h2>', retrieved.single.tabs[i].title, '</h2>',
        retrieved.single.tabs[i].date,'</div>',urlStr,'</div></div></div>']));       //critical to have all <div>(s) in correct closing, as in opening     
    }
    // console.log('str', str);
    str = str.reverse
    document.getElementById('submitHere').innerHTML = str;
